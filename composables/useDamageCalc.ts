...
const skillTable: SkillEntry[] = [
    ...
    { pts: 45, dmg: 180, prec: 0.45, crit: 0.45, critDmg: 0.90, arm: 0.36, esquive: 0.36, pv: 90, famine: 9 },
...
export function useDamageCalc() {
    const state = reactive({
        base:      { dmg: 100, prec: 0.5, crit: 0.1, critDmgPct: 0.5, arm: 0,   esquive: 0,   pv: 50 },
        bonus:     { dmg: 0,  prec: 0, crit: 0, critDmgPct: 0, arm: 0, esquive: 0, pv: 0 },
        skillCost: { dmg: 0,  prec: 0,   crit: 0,    critDmg:    0,    arm: 0,  esquive: 0,   pv: 0 }
    })

    // Calcule la stat totale : base + bonus + bonus de skill
    const combined = computed(() => {
        const total = (prop: keyof BaseStats) => {
            const bVal = state.base[prop]
            const eVal = state.bonus[prop]
            const key  = skillKeyMap[prop]
            const pts  = state.skillCost[key]
    ...
    // Formules basées sur `combined`
    const critDmg = computed(() => combined.value.dmg * (1 + combined.value.critDmgPct))

    const avgDmgPerHit = computed(() => {
        const c = combined.value
        return c.prec * ((1 - c.crit) * c.dmg + c.crit * (c.dmg * (1 + c.critDmgPct)))
    })
